...
*/
(function(){
    // Increase cap so the grid shows more shaders by default (avoid surprising truncation)
    var MAX = 48;
    ...
    function fetchShaders(){
        // Try multiple likely locations for the canonical JSON payload. If all fetches fail,
        // prefer any embedded `window.SHADERS_PUBLIC`, then attempt to dynamically load the
        // fallback script `Javascript/shadertoys/shaders_public_fallback.js`, and finally
        // fall back to the minimal `window.SHADERS_FALLBACK` list.
        var candidates = [
            'shadertoys/shaders_public.json',
            'shaders_public.json',
            'Javascript/shadertoys/shaders_public.json'
        ];

        function tryFetch(i){
            if (i >= candidates.length) {
                // If a full fallback JSON was embedded as JS, use it (window.SHADERS_PUBLIC)
                try { if (window && window.SHADERS_PUBLIC && window.SHADERS_PUBLIC.shaders && window.SHADERS_PUBLIC.shaders.length) { console.info('shaders_grid: using embedded window.SHADERS_PUBLIC; length=', window.SHADERS_PUBLIC.shaders.length); return Promise.resolve(window.SHADERS_PUBLIC); } } catch(e){}
                // Dynamically load the fallback script and wait for it to expose window.SHADERS_PUBLIC
                return new Promise(function(resolve){
                    try {
                        var s = document.createElement('script');
                        s.src = 'Javascript/shadertoys/shaders_public_fallback.js?v=' + Date.now();
                        s.onload = function(){ try { if (window && window.SHADERS_PUBLIC && window.SHADERS_PUBLIC.shaders && window.SHADERS_PUBLIC.shaders.length) { console.info('shaders_grid: loaded fallback script; length=', window.SHADERS_PUBLIC.shaders.length); resolve(window.SHADERS_PUBLIC); return; } } catch(e){}
                            // if fallback didn't populate, fall back to minimal
                            resolve({ shaders: (window.SHADERS_FALLBACK || []).map(function(f){ return { info: { name: f.title, id: f.id }, url: 'https://www.shadertoy.com/view/' + f.id }; }) }); };
                        s.onerror = function(){ resolve({ shaders: (window.SHADERS_FALLBACK || []).map(function(f){ return { info: { name: f.title, id: f.id }, url: 'https://www.shadertoy.com/view/' + f.id }; }) }); };
                        document.body.appendChild(s);
                    } catch (e) { resolve({ shaders: [] }); }
                });
            }
            return fetch(candidates[i]).then(function(resp){ if (!resp.ok) throw new Error('http status ' + resp.status); return resp.json(); }).catch(function(){ return tryFetch(i+1); });
        }

        return tryFetch(0).catch(function(){ // ultimate safe fallback
            ...
            if (fallback && fallback.length) return { shaders: fallback.map(function(f){ return { info: { name: f.title, id: f.id }, url: 'https://www.shadertoy.com/view/' + f.id }; }) };
            return { shaders: [] };
        });
    }
    ...
    function makeCard(item, idx)
    {
        ...
        function openOverlay()
        {
            ...
            // defensive inline styles to avoid live-site CSS overrides that push the overlay behind other content
            overlay.style.position = 'fixed'; overlay.style.inset = '0'; overlay.style.display = 'flex'; overlay.style.flexDirection = 'column'; overlay.style.alignItems = 'center'; overlay.style.justifyContent = 'center'; overlay.style.padding = '20px'; overlay.style.gap = '12px'; overlay.style.overflow = 'auto'; overlay.style.background = 'rgba(0,0,0,0.85)'; overlay.style.zIndex = '99999';
            var close = document.createElement('button'); close.className = 'shader-overlay-close'; close.textContent = '✕'; close.style.zIndex = '100000';
